...
vi path;
// s始まりのサイクルのパスを一つ求める
bool dfs(int s, int now, int pre = -1) {
  used[now] = 1;
  bool ret = false;
  for (auto &to: graph[now]) {
    ...
    if (used[to]) continue;
    ret |= dfs(s, to, now);
    if (ret) break; // サイクルが見つかったとき、それ以上の探索を打ち切る(出ないとsが二つのループに含まれているとき、二つともpathに加わってしまう。)
  }
  if (ret) path.pb(now);
  return ret; // サイクルがあるかどうかを返す
};

// VERIFICATION: CF_div2_649_D
// URL: https://codeforces.com/contest/1364/submission/83816478
